...
def copy_top_pdb(folder_name, folder_path, top_rows, pdb_dir):
    """
    Copy only the single best-scoring PDB into the shared pdb_dir,
    named {folder_name}_{description}.pdb.
    """
    score, desc = top_rows[0]
    src = os.path.join(folder_path, f"{desc}.pdb")
    dst = os.path.join(pdb_dir, f"{folder_name}_{desc}.pdb")

    if os.path.isfile(src):
        shutil.copy2(src, dst)
        return desc, "copied"
    else:
        return desc, "PDB not found"


def main():
    base = os.getcwd()

    # Single output folder at the base level
    pdb_dir = os.path.join(base, TOP_PDB_FOLDER)
    ...
    for folder in subfolders:
        ...
        desc, status = copy_top_pdb(folder, folder_path, top_rows, pdb_dir)
        processed += 1

        print(f"\n[{folder}] Top {TOP_N}:")
        for i, (score, d) in enumerate(top_rows, start=1):
            print(f"  {i}. {score:.3f}  {d}")
        print(f"  -> wrote {out_path}")
        print(f"  -> best PDB: {folder}_{desc}.pdb  [{status}]")

    print(f"\n-> Best PDBs collected in: {pdb_dir}")
